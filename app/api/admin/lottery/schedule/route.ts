...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!lotteryId) throw AppError.badRequest('lotteryId is required');
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!lotteryId || !scheduled_at) {
      throw AppError.badRequest('lotteryId and scheduled_at are required');
    }
    ...
    if (lError || !lottery) throw AppError.notFound('Lottery not found');
    if (lottery.status !== 'active') throw AppError.badRequest('Lottery is not active');
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Lottery fetch failure is non-blocking
    }
