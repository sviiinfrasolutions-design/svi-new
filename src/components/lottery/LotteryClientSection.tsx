...
export default function LotteryClientSection() {
  ...
  // While checking, show a minimal dark loading screen (matches section bg)
  if (loading) {
    return (
      <div className="flex min-h-[50vh] items-center justify-center bg-gradient-to-b from-[#0a0a0f] to-[#12121a]">
        <div className="flex flex-col items-center gap-3">
          <div className="border-brand-gold h-8 w-8 animate-spin rounded-full border-2 border-t-transparent" />
          <p className="text-[10px] font-bold tracking-widest text-gray-600 uppercase">Loading…</p>
        </div>
      </div>
    );
  }
